        ...
        foreach ($tabs as $id => $name) :
            ...
            foreach ($data[$id] as $eid => $page) :
                ...
                if (isset($prolet) && $prolet) {
                    $edit_url = $foundation->cpURL('publish', 'edit', array('entry_id' => $page['entry_id']));
                    $edit_url  = ee()->config->item('base_url') . $edit_url;
                    $site_id = ee()->config->item('site_id');
                    $edit_url .= "&site_id=" . $site_id . "&hide_closer=y" . "&preview=y";
                    if (array_key_exists($page['channel_id'], $assigned_channels)) echo '<a href="', $edit_url, '"target="_blank">', (!empty($_GET['debug']) ? $page['entry_id'] . ': ' : '') . $page['title'], '</a>';
                ...
                $specific_channel_rule = $vars['channel_rules'][$page['channel_id']];
