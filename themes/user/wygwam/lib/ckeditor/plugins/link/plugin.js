...
( function() {
	...
	CKEDITOR.plugins.link = {
		...
		parseLinkAttributes: function( editor, element ) {
			...
			if (!retval.type)
			{
				// Check to see if it's a custom link type
				var customLinkType = '';

				LINK_TYPES_LOOP: for ( var linkType in editor.config.link_types )
				{
					var links = editor.config.link_types[linkType];
					...
					for ( var i in links )
					{
						...
						if ( link.url == href )
						{
							retval.type = linkType
								.toLowerCase()
								.replace(' ', '_')
								.replace(/\'/g, '')
								.replace(/\"/g, '');

							retval.customLinkUrl = href;

							break LINK_TYPES_LOOP;
						}
					}
				}
			}
		...
		getLinkAttributes: function( editor, data ) {
			...
			switch ( data.type ) {
				...
				default:
					// Custom link type
					set[ 'data-cke-saved-href' ] = ( data.customLinkUrl || '' );
					break;
